...
import { DataAndParametersSelector, ResetStorageButton, StartText } from '@/components';
...
export function PolymerLinearization({ data }) {
  const {
  ...
  const prepareChartData = (xValues, yValues, coeffs, modelType, isCurrent = false) => {
    ...
    switch (modelType) {
      ...
      case 'relaxation':
        ...
        approxYValues = xValues.map(
          (x) => E_epsilon + (coeffs.sigma0 - E_epsilon) * Math.exp(-x / coeffs.theta),
        );
        break;
    }
    ...
    const items = [
      {
        ...
        children: (
          <div>
            ...
            <p>
              <strong>Уравнение:</strong> {result?.equation ?? '--'}
            </p>
            {modelType === 'maxwell' && (
              <div>
                <p>
                  <strong>Начальное напряжение σ₀</strong> = {result.sigma0?.toFixed(4) ?? 0}
                </p>
                <p>
                  <strong>Время релаксации τ</strong> = {result.tau?.toFixed(4) ?? 0}
                </p>
              </div>
            )}
            {modelType === 'power' && (
              <div>
                <p>
                  <strong>Коэффициент a</strong> = {result?.a?.toFixed(4) ?? 0}
                </p>
                <p>
                  <strong>Показатель степени b</strong> = {result?.b?.toFixed(4) ?? 0}
                </p>
              </div>
            )}
            {modelType === 'relaxation' && (
              <div>
                ...
                <p>
                  <strong>Характеристическое время θ</strong> = {result?.theta?.toFixed(4) ?? 0}
                </p>
                <p>
                  <strong>Модуль упругости E</strong> = {result?.E?.toFixed(4) ?? 0}
                </p>
                <p>
                  <strong>Деформация ε</strong> = {result?.epsilon?.toFixed(4) ?? 0}
                </p>
              </div>
            )}
          </div>
          ...
          <>
            ...
            <div className={s.history}>
              {linearizationHistory?.map((item, index) => (
                <div key={`${item.modelType}-${index}`} className={s.historyItemContainer}>
